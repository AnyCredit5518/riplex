...
- **Ctrl-C at a CLI prompt now raises immediately even when the async event loop has swallowed the first SIGINT.** The previous fix (narrowing the prompt except clauses to just ``EOFError`` so ``KeyboardInterrupt`` could propagate) was necessary but not sufficient: ``asyncio.run`` in Python 3.11+ installs its own SIGINT handler that cooperatively cancels the running task instead of raising ``KeyboardInterrupt`` on the first Ctrl-C. During a synchronous ``input()`` call inside an async flow that meant the first Ctrl-C got absorbed by asyncio's handler; on Windows the console read returned an empty string, and the prompt helpers happily treated that as "user pressed Enter" and used their default. So Ctrl-C at ``Title [Psych]:`` would advance to ``Resuming session:...`` — indistinguishable from a real Enter press. A new ``_input(prompt)`` helper in ``src/riplex/ui.py`` temporarily installs ``signal.default_int_handler`` (the standard raise-on-SIGINT handler) around each ``input()`` call and restores the asyncio handler after the read returns. All five prompt helpers (``prompt_choice``, ``prompt_confirm``, ``prompt_text``, ``prompt_multi_select``, ``prompt_proceed_or_edit``, ``prompt_rip_selection``) now go through ``_input`` so Ctrl-C raises immediately at any prompt regardless of whether we're inside ``asyncio.run``. ``signal.signal`` is only valid from the main thread, so the helper degrades gracefully (falls back to plain ``input``) when called off-thread (test runners, embedding).

- **Ctrl-C at a CLI prompt no longer silently continues as if the user pressed Enter.** All five interactive helpers in ``src/riplex/ui.py`` (``prompt_choice``, ``prompt_confirm``, ``prompt_text``, ``prompt_multi_select``, ``prompt_proceed_or_edit``, ``prompt_rip_selection``) previously caught ``KeyboardInterrupt`` alongside ``EOFError`` and returned the default value — so a user who hit Ctrl-C at ``Disc inserted and ready? [Y/n]`` would see the flow advance to ``Reading disc info ...`` as if they had confirmed. The intent had been to treat piped/non-interactive stdin (which raises ``EOFError`` from ``input()``) the same as accepting the default; ``KeyboardInterrupt`` got lumped in by accident. The except clauses now catch only ``EOFError``, so a Ctrl-C at a prompt bubbles up to the top-level ``main()`` handler that landed in the earlier Ctrl-C fix and prints ``Aborted (Ctrl-C).`` before exiting 130. Also fixes a copy-paste in ``tests/test_ui.py``: the ``test_eof_returns_default`` case for ``prompt_text`` had been throwing ``KeyboardInterrupt`` (not ``EOFError``) — so it was silently asserting the wrong behavior. That test now correctly throws ``EOFError`` and a new ``test_keyboard_interrupt_propagates`` case pins the new contract.
