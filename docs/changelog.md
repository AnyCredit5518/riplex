...
multi-season shows. The canonical test case is a complete-series TV boxset:
...
- **The "Select Titles to Rip" screen no longer raises a false multi-film alert, shows the confirmed movie title for the main feature, and has clearer table headers.** Four fixes to the GUI selection screen (and, for parity, the CLI `rip` warning). (1) The "Multi-film disc detected — N feature-length film(s)" alert was driven purely by dvdcompare's linked-film list (`detect_bonus_films`), which over-reports: a boxset's movies disc frequently hyperlinks sibling works (sequels, spin-offs) whose real home is a different disc, so a disc that physically presses a single feature (e.g. the linked feature film on disc 31) still raised the alert. A new pure `reconcile_bonus_films(bonus_films, rippable_titles, *, min_feature_seconds=2400)` in `disc/analysis.py` trims the list to at most `(feature-length rippable titles − 1)` entries, so a single-feature disc shows no alert; both `selection.py` and `riplex_cli/commands/rip.py` call it. (2) The alert's body claimed "per-film organization (separate Plex folders) is not yet wired up — move them manually", which is stale: `organize_by_group.build_multi_group_plan` (used by the GUI `organize_preview` and the CLI) already routes each film to its own Plex folder by runtime. The text now says riplex sorts each film into its own folder during organize. (3) The main feature's row displayed the raw classification label ("MAIN FILM (1080p)") instead of the title the user already confirmed on the release screen; it now shows the TMDb title (e.g. "Example Movie") and a highlighted indigo **MAIN FILM** badge next to the RIP/SKIP recommendation badge. (4) The title table's blank leading column is now headed "Rip?" and the RIP/SKIP badge column is headed "Recommendation" so it's unambiguous that the checkbox controls ripping while the badge is only a suggestion. Tests: 6 new cases in `tests/test_disc_analysis.py::TestReconcileBonusFilms`.

- **The Insert Disc screen no longer shows two Quit buttons.** ``main.navigate`` globally injects a Quit button into every non-welcome screen's footer ``Row`` (via ``_append_to_footer_row``), but ``disc_swap.py`` also built its own ``quit_btn`` and appended it to the same footer, so the Insert Disc screen rendered ``Quit`` twice. The screen's own button was removed along with its now-unused ``_quit`` handler; the remaining injected Quit is the better one anyway because it routes through ``_confirm_quit`` (a "Return to Welcome?" confirmation dialog) rather than navigating straight to Welcome.

- **The prepended primary-work film slot no longer shows "(unknown runtime)", and the disc overview now explains which discs a movie/season pick hid.** Two UX follow-ons to the multi-work boxset filtering work. (1) When ``group_release_discs`` prepends a dedicated ``FilmSlot`` for the picked primary work (the linked feature film on disc 31 case), it was constructing that slot with ``runtime_seconds=0`` because the only runtime source it had — ``current_tmdb_match`` — is a ``MetadataSearchResult``, which carries no runtime (runtime lives on ``MovieDetail`` / ``EpisodeMetadata``). The slot therefore rendered "(unknown runtime)" even though the runtime was known elsewhere. ``group_release_discs`` gained a keyword-only ``primary_runtime_seconds: int = 0`` parameter used for the prepended slot; the GUI passes ``app.state["movie_runtime"]`` (already stashed by the metadata screen on the fresh path) and the CLI passes ``LookupResult.movie_runtime``. Because the resume path never populated ``movie_runtime`` for movies, ``resume.py`` now fetches the TMDb movie detail (new ``_fetch_movie_runtime`` helper, mirroring ``_fetch_show_detail``) and ``ResumedLookup.movie_runtime`` flows into ``app.state`` via ``perform_resume_fetch``. (2) When the "one pick at a time" boundary filter drops discs — the 30 TV discs of a movie boxset pick, or cross-season discs of a single-season TV pick — the disc overview simply showed a shorter list with no explanation, which looked like riplex lost discs. Both filter boundaries (``release.py::_apply_season_filter`` on the fresh path, ``resume.py`` on the resume path) now record the dropped disc numbers; ``ResumedLookup`` gained a ``hidden_disc_numbers: list[int]`` field and both surfaces stash the list in ``app.state["hidden_disc_numbers"]``. A new ``_build_hidden_discs_banner`` on the disc-overview screen renders an informational card ("Hiding 30 of 31 discs — Discs 1-30 aren't part of the title you picked…") using a new pure ``format_disc_ranges`` helper in ``disc/analysis.py`` that collapses ``[1,2,3,4,9,10]`` to ``"1-4, 9-10"``. The "one pick at a time" filters themselves are unchanged. Tests: 6 new ``format_disc_ranges`` cases and 2 new runtime cases in ``tests/test_disc_analysis.py``, plus ``hidden_disc_numbers`` / ``movie_runtime`` assertions added to ``tests/test_resume.py::test_movie_boxset_drops_primary_work_discs``.

- **Movie picks against a multi-work boxset now filter down to the movie disc(s) on the resume path too, and the picked movie appears as its own film slot alongside the linked works.** Two follow-on issues to the previous entry surfaced when testing the complete-series TV boxset scenario end-to-end. (1) The GUI resume path — triggered whenever ``find_existing_session`` finds a marker for the inserted disc's title — routes through ``_resume_session`` → ``riplex.resume.resume_from_session`` and hands the freshly-``_convert_release``'d discs straight back to the caller. That adapter had no filtering step at all, so a resume of a movie pick against the complete-series boxset would still hand all 31 discs to the disc-overview screen even though the fresh-lookup path in ``release.py::_use_release`` filtered correctly. The adapter now mirrors the fresh-lookup path: after ``_convert_release`` succeeds it calls ``filter_discs_to_picked_movie`` for movie picks (dropping empty-pointer primary-work discs) or ``filter_discs_to_season`` for TV picks with a known season (which had also been missing here — a TV resume on a Complete-Series release would have leaked cross-season discs into overview and rip planning). ``ResumedLookup`` gained a ``primary_movie_needs_slot: bool`` field that ``_resume_session`` copies into ``app.state`` under the same key. (2) With the filter applied, disc 31 was the only remaining disc — but ``group_release_discs`` then created one group holding the 2 pointered ``FilmSlot``s (Example Movie 2 / Example Movie 3) and hit its "all groups have pointered slots" fallback, which *overwrote* the first pointered slot's ``tmdb_match`` with the picked linked feature film's match. The picked movie belongs on the same disc — it's "The Film" main feature of disc 31 alongside the two hyperlinked sequels — but overwriting the second linked film's slot was clearly wrong. ``group_release_discs`` gained a keyword-only ``add_primary_work_slot: bool = False`` parameter; when True and the fallback branch fires, it now **prepends** a new ``FilmSlot`` (titled from the picked TMDb match) to the first group instead of clobbering the first pointered slot. The label rewrites to ``"Disc N: K works"`` so the group heading reflects the added slot. Both ``disc_overview.py`` and ``lookup.py::resolve_disc_groups`` read the flag off state / ``LookupResult.primary_movie_needs_slot`` and pass it through, so GUI and CLI both surface a disc with three slots: the primary movie plus the two linked works. State also gets ``primary_movie_needs_slot=False`` written on every call to ``_apply_season_filter`` so a subsequent TV pick doesn't inherit a stale movie flag. Tests: 3 new cases in ``TestGroupReleaseDiscs`` (prepend-when-flag-set, off-by-default, ignored-when-primary-group-exists), 1 new case in ``TestResumeFromSession`` covering the movie-boxset shape (30 primary discs dropped + flag set), and the existing ``test_matches_release_by_name`` was updated to pass real ``PlannedDisc`` objects through the mocked ``_convert_release`` since the new filter step exercises ``PlannedDisc.number``.

- **Movie picks against a multi-work boxset now filter down to just the movie disc(s) instead of grouping the picked movie under the boxset's TV episodes.** When a disc from a mixed-content boxset was inserted (e.g. a complete-series TV boxset — 30 discs of TV episodes plus 1 disc of standalone TV-movie sequels) and the user picked a movie in the metadata screen, the disc overview would show ``Discs 1-30`` labelled with the picked movie (because ``group_release_discs`` auto-assigns the current TMDb match to the first empty-films group, which was the primary-work TV run) while the actual movie disc — Disc 31 — was labelled ``2 linked works``. Regression fell out of the ``select season`` / release-picker refactor: the TV path had grown a ``filter_discs_to_season`` shortcut that dropped cross-season discs before ``group_release_discs`` ran, but the movie path had no equivalent — ``_apply_season_filter`` in ``src/riplex_app/screens/release.py`` early-returned for any ``media_type != "tv"`` and ``src/riplex/lookup.py``'s season-filter branch was gated by ``not is_movie``. Both surfaces then handed the full 31-disc release into ``group_release_discs``, which assigns the picked movie to the first group without film slots and produces the wrong labels. Fix adds ``filter_discs_to_picked_movie(discs)`` to ``src/riplex/disc/analysis.py`` — a movie counterpart to ``filter_discs_to_season`` — which keeps only discs whose extras carry a ``pointer_fid`` (dvdcompare's hyperlink to another film page, the marker for a bonus/standalone-work extra) when any such discs exist, and passes the input through unchanged for plain single-work movie releases (no pointers anywhere). Both call sites now invoke it: ``_apply_season_filter`` gains a ``movie`` branch, and ``lookup.py``'s filter block adds a mirror ``elif is_movie and discs`` call. Tests: 5 new cases in ``tests/test_disc_analysis.py::TestFilterDiscsToPickedMovie`` covering empty input, the multi-work boxset (30 empty-pointer + 1 pointer disc → keeps only disc 31), a plain movie release with featurette-only extras (unchanged), discs with no extras at all (unchanged), and an all-pointer release (unchanged).

- **Ctrl-C at a CLI prompt now aborts immediately instead of silently behaving like Enter.** Two independent bugs conspired to make Ctrl-C at any prompt (``Title [Example Show]:``, ``Disc inserted and ready? [Y/n]``, the disc-selection picker, etc.) silently advance the flow using the prompt's default value. First, all five interactive helpers in ``src/riplex/ui.py`` caught ``KeyboardInterrupt`` alongside ``EOFError`` and returned the default — the intent had been to treat piped/non-interactive stdin the same as accepting the default, and ``KeyboardInterrupt`` got lumped in by accident. Second, ``asyncio.run`` in Python 3.11+ installs its own SIGINT handler that cooperatively cancels the running task instead of raising ``KeyboardInterrupt`` on the first Ctrl-C; during a synchronous ``input()`` call inside the async orchestrate flow that meant the first Ctrl-C was absorbed by asyncio's handler, and on Windows the console read returned an empty string that looked exactly like the user pressing Enter. Fix is two changes. (1) The prompt helpers now catch only ``EOFError`` so ``KeyboardInterrupt`` propagates. (2) ``main()`` in ``src/riplex_cli/main.py`` no longer uses ``asyncio.run``: it drives ``loop.run_until_complete`` on a fresh event loop directly, which leaves Python's default SIGINT behavior in place (raise ``KeyboardInterrupt`` in the main thread immediately) instead of installing asyncio's cooperative-cancel handler. Also fixes a copy-paste in ``tests/test_ui.py`` where ``prompt_text``'s ``test_eof_returns_default`` had been throwing ``KeyboardInterrupt`` (not ``EOFError``) — so it was silently asserting the wrong behavior — and adds ``test_keyboard_interrupt_propagates`` to pin the new contract. ``troubleshooting.md`` was updated to drop the "requires two Ctrl-Cs" note that's no longer accurate.

### Changed

- **CLI orchestrate now resumes from an existing session marker the same way the GUI does.** Previously ``riplex orchestrate`` would always re-run the full TMDb + dvdcompare lookup even when a ``_riplex_session.json`` marker for the title already sat next to the rip root, meaning a resume on Disc 2 of a partially-ripped set would re-prompt for season, re-fetch TMDb, and re-pick the dvdcompare release even though every one of those decisions had already been made and persisted on Disc 1. The GUI shortcut for this had been living inside ``DiscDetectionScreen._fetch_dvdcompare_for_resume`` — a 90-line block that read the marker, ran a best-guess TMDb rehydration for legacy markers without ``source_id``, fetched the dvdcompare film, matched the release by name, converted the release into ``PlannedDisc``s, and backfilled the season number from the film title. That flow logic has been extracted into a new shared adapter ``src/riplex/resume.py`` with a single async entry point ``resume_from_session(session, *, disc_info=None) -> ResumedLookup``. The GUI method is now a 40-line thin wrapper that just calls the adapter and copies the returned fields into ``app.state``; the CLI's ``orchestrate.py`` calls ``find_existing_session(title_arg)`` right after title confirmation and — when a marker is found — awaits the same adapter, skipping the entire TMDb + dvdcompare block in favor of the rehydrated ``ResumedLookup``. Behavior parity is exact: identical film-lookup call (same disc-format fallback via ``detect_disc_format`` when the marker didn't record one), identical release-selection heuristic (match ``session.release_name`` first, fall back to the first release when no name match), identical legacy-marker fallback (best-guess TMDb search when ``source_id`` is empty). On the CLI the resume prints ``Resuming session: <title> (<year>) — N disc(s) already ripped`` to stderr and populates ``args.season_number`` and ``disc_format`` from the marker when the user didn't pass them explicitly. Also added a ``## Shared flow logic (CLI/GUI parity)`` section to ``.github/copilot-instructions.md`` documenting the discipline: multi-step flow logic lives in ``src/riplex/`` as plain functions returning dataclasses; ``src/riplex_cli/`` and ``src/riplex_app/`` become thin callers so future features can't drift between the two surfaces. Tests: 8 new cases in ``tests/test_resume.py`` covering the release-name match, first-release fallback, movie season-skip, disc-format fallback from ``disc_info``, film-fetch failure, ``_convert_release`` failure, legacy marker stub tmdb_match, and empty-releases branch. Full suite now 911 passing (was 903).

- **CLI now exits cleanly on Ctrl-C instead of dumping a Python traceback, and no longer orphans `makemkvcon` when the user force-quits a rip.** Previously the top-level ``main()`` never caught ``KeyboardInterrupt`` — a Ctrl-C during ``riplex orchestrate`` (or any long-running subcommand) would bubble a 20-line traceback ending in ``asyncio.runners._on_sigint`` to the user's terminal. And because ``MakeMKV.rip()``'s except clause only caught ``Exception`` (not ``BaseException``), a Ctrl-C during a rip would leave the ``makemkvcon`` subprocess running as an orphan while Python's own traceback was being printed. The exit now shows a single ``Aborted (Ctrl-C).`` line to stderr and exits with code 130 (the conventional SIGINT termination code); the rip loop's except clause is widened to catch ``KeyboardInterrupt`` so ``proc.kill()`` runs on the way out. A ``troubleshooting.md`` entry now explains why Ctrl-C during a rip can take a moment to take effect (Python blocks in a C-level ``read()`` on the subprocess pipe until makemkvcon yields) and how to force-quit if needed.

### Changed

- **Disc-mismatch warning now uses softer, more honest wording in both CLI orchestrate and GUI disc-swap.** When ``detect_disc_number`` returned a different disc than expected, the old message read ``Warning: expected Disc M but detected Disc N`` — which implies the detector is authoritative and that the user inserted the wrong disc. In practice this fires constantly on TV series because ``detect_disc_number`` falls through to strategy 3 (title-runtime matching) when the volume label has no ``_DN`` token and no format-based signal distinguishes discs (all-Blu-ray release), and runtime matching cannot distinguish, e.g., a procedural TV series' Season 2 Disc 1 (six ~44min episodes) from Disc 2 (also six ~44min episodes). The CLI now says ``Note: this disc's title runtimes look more like Disc N than Disc M. Auto-detection is unreliable when episode lengths are similar between discs (common for TV shows) — if you inserted the correct disc, continue.`` The GUI equivalent on ``disc_swap.py`` similarly says ``Note: runtimes look more like Disc N than Disc M (auto-detection is unreliable on TV discs with similar episode lengths). Proceeding with Disc M.`` The ``Continue anyway?`` prompt on the CLI side is retained because the check is still useful in a small number of movie scenarios (a set with a bonus disc where the user grabbed the feature disc by mistake), and the confirm defaults to ``yes`` so pressing Enter no longer feels like an escape from an error.
...
- **dvdcompare auto-lookup no longer picks alphabetically-first film of the wrong franchise.** Searching a short common title like "Example Show" against dvdcompare returns 100+ results; the previous format-only ranker picked "Unrelated Film (Blu-ray)" (alphabetically first Blu-ray match) instead of "Example Show: Season 1 (TV) (Blu-ray)". Auto-selection now filters results to those whose leading title (before the first colon) exactly matches the query, so an unrelated franchise sharing a substring can never win over an actual title match. The format check also falls back to scanning the result title text since dvdcompare's scraper sometimes leaves the `disc_format` field unset for Blu-ray entries.
- **Season label for the leading discs of a series release now falls back to the film title.** Pages like `Example Show: Season 1 (TV) (Blu-ray)` (fid=66231) use `DISC ONE ... DISC FOUR` for the release's own discs (no explicit `Season 1:` header) and `DISCS FIVE - EIGHT: Season 2` for the pointer runs. The scraper faithfully returns empty titles for discs 1–4, which left the disc overview without a season chip on those rows even though the film title itself says Season 1. `build_season_labels` now parses `Season N` out of the film title and applies it to the leading untitled run; explicit later runs (Season 2, 3, …) still win, and trailing untitled discs (bonus platters) stay unlabeled.
- **Bonus-films disc on a season-entry Complete Series release is now split into its own group.** On the `Example Show: Season 1` dvdcompare page (fid=66231), disc 31 lists three standalone TV-movies via `<a href="film.php?fid=…">` links but doesn't carry a `* The Film` marker — so the previous grouping treated disc 31 as another TV disc and each ~90-minute movie as an unlabeled episode. `dvdcompare-scraper` now captures the anchor's target fid on `Feature.pointer_fid`; riplex threads it through to `PlannedExtra.pointer_fid` and treats any disc with pointered extras as film-like. Each film becomes its own `FilmSlot` carrying the linked fid, and Disc Overview autofill hits that fid to read the canonical film title before searching TMDb — so the three linked films now auto-fill into their own Plex movie folders regardless of which physical disc the user inserted first.
...
- **Per-group rip output on the Select Titles screen.** In orchestrate mode the selection screen now looks up the current disc's DiscGroup and swaps in that group's TMDb match for the header title, the season/disc chip, and the rip output folder. Previously all discs in a multi-work release ripped under whichever match the user picked at the metadata screen, so a release (TV series + bonus films disc) would send every disc under `Example Movie (2017)/` regardless of what was actually on the platter.
